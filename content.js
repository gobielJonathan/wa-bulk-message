...
if (typeof window.waSenderInjected === 'undefined') {
    ...
    async function executeWhatsAppWorkflow(phone, messageText) {
        ...
        const newChatBtn = await waitForElement('button[aria-label="New chat"], span[data-icon="new-chat-outline"]', 5000);
        ...
        const searchInput = await waitForElement('input[aria-label="Search name, number or username"], div[contenteditable="true"][data-tab="3"]', 5000);
        ...
        const clickableTarget = await waitForElement('[data-testid="cell-frame-container"]', 8000);
        ...
        const messageBox = await waitForElement('footer div[contenteditable="true"]', 8000);
